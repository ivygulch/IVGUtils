...
@interface NSString_IVGUtilsTest : XCTestCase
@end
...
- (void) testUUIDUniqueness {
    ...
    XCTAssertNotEqualObjects(guid1, guid2, @"guid strings should be unique");
}

- (void) check:(NSString *) s expectHaveValue:(BOOL) expectHaveValue {
    BOOL actualHaveValue = [s hasValue];
    XCTAssertEqual(actualHaveValue, expectHaveValue, @"string is %@", s);
}
